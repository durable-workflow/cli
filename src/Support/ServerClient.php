...
class ServerClient
{
    ...
    private function request(string $method, string $path, array $options = []): array
    {
        ...
        try {
            $response = $this->http->request($method, $this->requestUrl($path), $options);
    ...
    private function httpException(int $statusCode, array $body, string $path): ServerHttpException
    {
        ...
        if ($statusCode === 403 && $this->isManagedNamespaceRuntimeUrl()) {
            $message .= self::isWorkerProtocolPath($path)
                ? ' This worker-protocol operation requires a worker runtime credential; a client runtime credential cannot be used.'
                : ' This client/control-plane operation requires a client runtime credential; worker runtime credentials are limited to worker-protocol operations.';
        }
    ...
    private function isManagedNamespaceRuntimeUrl(): bool
    {
        $path = parse_url($this->baseUrl, PHP_URL_PATH);

        return is_string($path)
            && preg_match('#/api/runtime/v1/namespaces/[^/]+$#', $path) === 1;
    }
